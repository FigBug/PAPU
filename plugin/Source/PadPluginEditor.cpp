...
void PAPUAudioProcessorEditor::resized()
{
    ...
    controls[n - 1]->setBounds (getGridArea (12, 3));
    controls[n - 2]->setBounds (getGridArea (13, 3));
    controls[n - 3]->setBounds (getGridArea (10, 3));
    controls[n - 4]->setBounds (getGridArea (11, 3));

    auto rcScope = getGridArea (10, 0, 3, 3).reduced (5);
    scope.setBounds (rcScope.withRight (getWidth() - inset - 5));

    auto rcKeyboard = getLocalBounds().reduced (5);
    keyboard.setBounds (rcKeyboard.withTop (rcScope.getBottom() + 130));
}
